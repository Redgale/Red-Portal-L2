...
//   window.open() inside a .then() callback is async, so the browser
//   treats it as a "scripted" popup and blocks it after any prior failure.
//   Once blocked, every subsequent call fails too → cascade.
//
// Fix: call window.open() IMMEDIATELY on the click event (sync),
//   show a loading screen, fetch in the background, then either:
//     ✓ Inject base-tagged blob URL if fetch succeeds
//     ✗ Fall back to direct navigation if fetch fails (CORS / network)
//   Either way, the game opens.
//
function openGame(url) {
  if (!url) { toast('No URL set for this game.', 'error'); return; }
  if (!/^https?:\/\//i.test(url)) url = 'https://' + url;

  // SYNC: open popup immediately tied to user gesture
  ...
  // Immediately fill popup so it looks active
  win.document.open();
  ...
  fetch(url, { signal: controller.signal })
    ...
    .catch(() => {
      clearTimeout(tid);
      // Graceful fallback — just go directly, no error shown to user
...
// ── Website Fetcher (same fix applied) ──────────────────────────────
function fetchSite() {
